...
<div class="flex items-center justify-end">
    <a href="{{ route('log-approval--excel') }}" type="button" class="btn btn-primary">Export</a>
</div>
    ...
    <div x-data="multipleTable">
        <div class="panel mt-6">
            <h5 class=" mb-5 font-semibold text-lg dark:text-white-light">Log Approval</h5>
                    ...
                    this.datatable2 = new simpleDatatables.DataTable('#myTable2', {
                        data: {
                            headings: ['Booker Name', 'Purpose', 'Vehicle', 'Vehicle Condition', 'Booking Start',
                                'Booking End', 'Approved By', 'Approved By','Status'
                            ],
                            data: dataJson
                        },
                        searchable: true,
                        ...
                        columns: [
                            {
                                ...
                                render: (data, cell, row) => {
                                    ...
                                    output += ` <button class="badge text-sm bg-${color[data.approve[0].status]} flex items-center justify-between" @click="$dispatch('open-modal-note',{name:'${data.approve[0].users.name}',note:'${data.approve[0].note}',status:'${data.approve[0].status}'})">Info</button>`;
                            ...
                            },
                            {
                                ...
                                render: (data, cell, row) => {
                                    ...
                                    output += ` <button class="badge text-sm bg-${color[data.approve[1].status]} flex items-center justify-between" @click="$dispatch('open-modal-note',{name:'${data.approve[1].users.name}',note:'${data.approve[1].note}',status:'${data.approve[1].status}'})">Info</button>`;
                            ...
                            },
                            {
                                ...
                                render: (data, cell, row) => {
                                    var color = ['warning','success','danger'];
                                    var message = ['Wait','Accept','Decline'];
                                    return '<span class="badge text-sm bg-' + color[data] + '">' + message[data] +
                                        '</span>';
                                },
                            },

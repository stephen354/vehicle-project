...
@endif
    ...
    <script src="/assets/js/simple-datatables.js"></script>
            ...
            <div class="flex items-center justify-end">
                <button type="button" class="btn btn-primary" @click="$dispatch('open-modal', {id:null,name:'',email:'',role:'',action:'Add'})">Create</button>
            </div>

              <div x-data="{ open: false , id:null,name:'',email:'',role:null,action:''}" x-on:open-modal.window="open = true; id = $event.detail.id;name = $event.detail.name;email = $event.detail.email;role = $event.detail.role;action = $event.detail.action;" x-on:close-modal.window="open = false" class="mb-5">
                            ...
                            <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                <div x-show="open" x-transition x-transition.duration.300
                                    ...
                                    <div
                                        ...
                                        <div class="font-bold text-lg"><span x-text="action">Add</span> User</div>
                                    ...
                                    <div class="p-5">
                                        <div class="dark:text-white-dark/70 text-base font-medium text-[#1f2937]">
                                                ...
                                                <input type="text" name="user_id" x-model="id" class="form-input mb-3"hidden/>
                                                <input type="text" name="name" x-model="name" placeholder="Name ..." class="form-input mb-3" required />
                                                <input type="text" name="email" x-model="email" placeholder="Email..." class="form-input mb-3" required />
                                                <input type="password" name="password" placeholder="Password..." class="form-input mb-3" required />
                            
                                                    <select class="form-select mb-3" name="role" x-model="role">
                                                        <option value="1">Admin</option>
                                                        <option value="2">Supervisor</option>
                                                        <option value="3">Manager</option>
                                                        <option value="4">Fleet Manager</option>
                                                        <option value="5">Logistics Manager</option>
                                                    </select>
                                        ...
                                        <div class="flex justify-end items-center mt-8">
                                            ...
                                            <button type="submit" class="btn btn-primary ltr:ml-4 rtl:mr-4" x-text="action">Submit</button>
                                        </div>
                                    </div>
                                    </form>
                                </div>
    ...
    <div x-data="multipleTable">
        <div class="panel mt-6">
            <h5 class="md:absolute md:top-[25px] md:mb-0 mb-5 font-semibold text-lg dark:text-white-light">Users</h5>
            <table id="myTable2" class="whitespace-nowrap"></table>
        </div>
    </div>

    <script>
        const dataUsers = @json($data).map(user => [
                user.name,            
                user.email,          
                user.role,          
            ...
            Alpine.data("multipleTable", () => ({
                ...
                init() {
                    ...
                    this.datatable2 = new simpleDatatables.DataTable('#myTable2', {
                        data: {
                            headings: ['Name', 'Email', 'Role', 'Action'],
                            data: dataUsers
                        },
                        searchable: true,
                        perPage: 10,
                        ...
                        columns: [
                            {
                                ...
                                render: (data, cell, row) => {
                                    var output = ["Admin","Supervisor","Manager","Fleet Manager","Logistic Manager"];
                                    var color = ['primary', 'secondary', 'success','warning','info'];
                                    return '<span class="badge text-sm bg-' + color[data-1] + '">' + output[data-1] +
                                        '</span>';
                                },
                            },
                            {
                                ...
                                render: (data, cell, row) => {
                                    ...
                                    return `<div class="flex items-center">
                                            <button type="button" class="ltr:mr-2 rtl:ml-2" x-tooltip="Edit" @click="$dispatch('open-modal', {id:${d.id},name:'${d.name}',email:'${d.email}',role:'${d.role}',action:'Edit'})" data-id="1" data-name="">
